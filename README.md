...
5. The example Docker command maps host port `8000` to container port `7860`, so you can still use `http://localhost:8000` locally.
...
Or point it at a local Docker image:
...
$env:MODEL_NAME="Qwen/Qwen2.5-72B-Instruct"
$env:HF_TOKEN="<your-token>"
$env:LOCAL_IMAGE_NAME="support-ops-env:local"
uv run inference --output inference_results.json
```
